 ...
 */
package com.hvn.wireguarddemo.wireguard.viewmodel
...
class PeerProxy : BaseObservable, Parcelable {
    ...
    fun setExcludingPrivateIps(excludingPrivateIps: Boolean) {
        ...
        for (network in input) {
            if (oldNetworks.contains(network)) {
                if (!replaced) {
                    for (replacement in newNetworks) if (!output.contains(replacement)) output.add(
                        replacement
                    )
                    replaced = true
                }
        ...
        allowedIpsState =
            if (excludingPrivateIps) AllowedIpsState.CONTAINS_IPV4_PUBLIC_NETWORKS else AllowedIpsState.CONTAINS_IPV4_WILDCARD
        notifyPropertyChanged(BR.allowedIps)
        notifyPropertyChanged(BR.excludingPrivateIps)
    }
    ...
    private fun setInterfaceDns(dnsServers: CharSequence) {
        ...
        if (allowedIpsState == AllowedIpsState.CONTAINS_IPV4_PUBLIC_NETWORKS) {
            ...
            val output = input.filter { !dnsRoutes.contains(it) || newDnsRoutes.contains(it) }
                .plus(newDnsRoutes).distinct()
    ...
    fun unbind() {
        ...
        val interfaze: InterfaceProxy = owner!!.`interface`
        val peers = owner!!.peers
        if (interfaceDnsListener != null) interfaze.removeOnPropertyChangedCallback(
