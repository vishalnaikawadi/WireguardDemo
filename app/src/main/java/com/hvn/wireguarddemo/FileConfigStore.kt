...
class FileConfigStore(private val context: Context) : ConfigStore {
    ...
    override fun load(name: String): Config {
        /*FileInputStream(fileFor(name))*/ context.assets.open("$name.conf").use { stream -> return Config.parse(stream) }
    }
    ...
    override fun rename(name: String, replacement: String) {
        ...
        if (!file.renameTo(replacementFile)) {
            if (!replacementFile.delete()) Log.w(
                TAG,
                "Couldn't delete marker file for new name $replacement"
            )
